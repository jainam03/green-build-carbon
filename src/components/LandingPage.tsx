...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-accent/20 selection:text-foreground">
      {/* Skip-to-content link (Nielsen H7 / Accessibility) */}
      ...
      <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="flex justify-between items-center px-6 md:px-8 py-4 max-w-[1400px] mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Mobile Hamburger (Nielsen H7 — previously missing entirely!) */}
            <button
          ...
          {mobileMenuOpen && (
            ...
            >
              <div className="px-6 py-6 space-y-1">
                ...
                <MobileNavLink onClick={() => { document.getElementById('features-section')?.scrollIntoView({ behavior: 'smooth' }); setMobileMenuOpen(false); }}>
                  Methodology
                </MobileNavLink>
                ...
                {/* Solutions Section */}
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2 px-3">Solutions</p>
                {solutions.map((s) => (
                  <MobileNavLink key={s.title} onClick={() => { navigate(s.href); setMobileMenuOpen(false); }}>
                    {s.title}
                  </MobileNavLink>
                ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            >
              ...
              <motion.p variants={fadeUp} className="text-background/70 text-lg mb-8 leading-relaxed">
                Trace Carbon's flagship Carbon Dashboard is the operational center for structural footprint analysis. Engineered for developers and authorities, it calculates site-specific emissions down to the logistics level.
              </motion.p>

              <ul className="space-y-6">
                {[
                  { icon: Activity, title: "Plain English LCA", desc: "Translates complex LCA science into plain English with intuitive inputs like Square Footage." },
                  { icon: SlidersHorizontal, title: "Chronological Trendlines", desc: "Provides real-time chronological trendlines and transparent data alignment." },
                  { icon: ShieldCheck, title: "Actionable Insights", desc: "Empowers demolition teams with actionable insights to optimize site operations and reduce emissions." },
                ].map((item, i) => (
      ...
      <section id="stakeholders-section" className="py-24 md:py-32 max-w-[1400px] mx-auto px-6 md:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-muted-foreground text-lg leading-relaxed">
            Sustainability isn't solved in silos. TraceCarbon connects the entire C&D ecosystem—from site supervisors to recycling vendors.
          </p>
        </div>
          ...
          <NavigationMenuContent>
            <ul className="grid gap-3 p-6 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr] border-border bg-popover shadow-elevated">
              ...
              <ListItem href="/" title="Vendor Integration">
                How waste management vendors plug into Trace Carbon.
              </ListItem>
